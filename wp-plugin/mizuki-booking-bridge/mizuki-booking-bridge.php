...
define( 'MIZUKI_BRIDGE_VERSION', '1.15.0' );
...
function mizuki_register_assets() {
	...
	// The shop pages — Mizuki Picks and Tools & Vases — share one stylesheet and one script, and
	// the product page's faces: all three sit next to each other in the shop, and a second set of
	// near-identical greens or a second Google Fonts request would both show.
	wp_register_style( 'mizuki-shop-page', $base . 'css/shop-page.css', $product_fonts, mizuki_asset_version( 'css/shop-page.css' ) );
	wp_register_script( 'mizuki-shop-page', $base . 'js/shop-page.js', array(), mizuki_asset_version( 'js/shop-page.js' ), true );
}
...
function mizuki_render_widget( $atts, $view, $extra = array() ) {
	...
	$attributes = array(
		...
		'data-view'           => $view,
		/*
		 * The studio's mark, served from this plugin rather than from the booking system.
		 *
		 * Same origin as the page it appears on, so no third request to another host and
		 * nothing to break if the studio reorganises their media library.
		 */
		'data-logo'           => plugin_dir_url( MIZUKI_BRIDGE_FILE ) . 'mizuki-logo.png',
	);
